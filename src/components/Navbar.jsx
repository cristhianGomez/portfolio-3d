...
import { logo, menu, close, logoTitle } from '../assets' 
    ...
    <nav className="w-full flex items-center py-5 fixed top-0 z-20 bg-white shadow-[0_4px_4px_rgba(229,237,262,1)]"> 
      <div className={`${styles.paddingX} w-full flex justify-between items-center max-w-7xl mx-auto`}>
        <Link to="/" className="flex items-center gap-2" onClick={() => { setActive(""); window.scrollTo(0,0);}}>
          ...
          <img src={logoTitle} alt="logoTitle" className="w-auto  object-contain" />
        </Link>
        <ul className="list-none hidden sm:flex flex-row gap-10">
          {navLinks.map(link => (
            <li
              key={link.id}
              className={`${active === link.title ? "text-white" : "text-primary"} hover:text-secondary text-[18px] font-semibold cursor-pointer`}
